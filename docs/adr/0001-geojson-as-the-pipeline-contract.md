...
Stage 1, the Normalizer, is the only OSM-aware component. It streams OSM/XML, emits
GeoJSON, and translates raw OSM tags into a stable normalized Feature property schema
(`name`, `category`, `importance`, ...). Stage 2, the Generator, is OSM-agnostic: it
consumes GeoJSON, whether from the Normalizer or any other source, and ranks and draws
using only the normalized schema.
...
- Stage 2 can be a lean, portable library (see ADR-0004 stack note) suitable for later
  in-game use, decoupled from the heavy parser.
...
- GeoJSON can't represent OSM topology (shared-node connectivity). That's fine for v1
  but constrains the deferred road-network feature; see ADR-0003.
