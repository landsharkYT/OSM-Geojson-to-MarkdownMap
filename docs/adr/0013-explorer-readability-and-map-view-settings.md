...
The SVG view became unreadable on a real extract: everything lived in one scaled `<g>`, so
zooming magnified the dots and labels instead of separating them; ~130 labels overlapped;
the gray proximity graph read like a street network; barriers and "crosses-barrier" edges
were both red dashes; and the convex-hull terrain (ADR-0008) looked like an inaccurate
shoreline.
...
- Terrain precision isn't pursued (ADR-0008 stands); we present it honestly instead. If
  precise terrain is ever needed, that's a separate decision reversing 0008.
