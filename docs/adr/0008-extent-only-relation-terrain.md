...
Water bodies and large parks live in OSM **multipolygon relations** (e.g. a lake is
`relation natural=water type=multipolygon`), not ways — which is why the ways-only Stage 1
(step 4) emitted no water. A relation's boundary is split across member ways that must be
stitched into rings, and large bodies are **clipped at the export bbox**, so a true ring
may not even close. Faithful ring assembly is fragile exactly where it matters.
